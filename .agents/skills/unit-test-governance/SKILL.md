...
3. **L1 策略案例**：从 `{paths.governance.test_strategy}/ST-{scope}-测试策略_v{version}_{date}.md`（路径和命名从配置读取）的「3.1 L1 后端单测」章节提取推荐的单测案例。
...
1. **确认输入**：提取或推断代码范围、测试文件列表和 L1 策略案例，经用户确认后继续。如果输入是 `{paths.governance.test_strategy}/ST-{scope}-测试策略_v{version}_{date}.md`（路径和命名从配置读取），从中提取代码范围、测试文件、自测检查点映射和 L1 策略案例。
2. 识别变更中的可执行代码。
3. 在判断缺少覆盖前，先搜索工程中已有单测文件。
4. 根据路径、模块名、函数名、类名、接口路由、功能名和测试标题，把变更代码映射到相关测试。
5. **检查 L1 策略案例覆盖**：将 `{paths.governance.test_strategy}/ST-{scope}-测试策略_v{version}_{date}.md`（路径和命名从配置读取）的「3.1 L1 后端单测」章节推荐的 L1 单测案例与现有单测进行对比，判断每个案例是否已覆盖。
6. 如果存在相关单测且环境可执行，优先运行最小相关测试命令。
7. 如果存在覆盖率命令，运行或请求最小相关的增量覆盖率命令。
8. 读取 `references/self-test-checkpoints.md`，只筛选自测检查点快照中适合 L1 单测治理的内容。
9. 将变更逻辑分类为：必须覆盖、可排除、单测已覆盖、或需要非单测覆盖。
10. 检查单测执行结果和通过率。
11. 检查增量覆盖率结果。
12. 输出单测门禁摘要、已有测试证据、L1 策略案例覆盖检查、覆盖分类、自测检查点 L1 覆盖现状、需补充单测清单、未覆盖风险。
13. 给出最终结论：通过、不通过或阻塞。
...
从 test-strategy 报告的「3.1 L1 后端单测」章节提取推荐的单测案例，检查每个案例是否已被现有单测覆盖。
