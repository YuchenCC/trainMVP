...
- 用户提供的文件列表。
- 一个模块或包。
- `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md`（从中提取代码范围、测试文件、L1策略和自测检查点映射）。
...
1. **代码范围**：从 `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md` 提取，或从用户输入、分支、commit range、文件列表推断。
2. **测试文件列表**：从 `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md` 提取，或搜索工程中相关单测文件。
3. **L1 策略案例**：从 `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md` 的「3.1 L1 后端单测」章节提取推荐的单测案例。
...
1. **确认输入**：提取或推断代码范围、测试文件列表和 L1 策略案例，经用户确认后继续。如果输入是 `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md`，从中提取代码范围、测试文件、自测检查点映射和 L1 策略案例。
2. 识别变更中的可执行代码。
3. 在判断缺少覆盖前，先搜索工程中已有单测文件。
4. 根据路径、模块名、函数名、类名、接口路由、功能名和测试标题，把变更代码映射到相关测试。
5. **检查 L1 策略案例覆盖**：将 `reports/test-strategy/requirement-{scope}-测试策略与覆盖分析.md` 的「3.1 L1 后端单测」章节推荐的 L1 单测案例与现有单测进行对比，判断每个案例是否已覆盖。
