...
按配置读取默认位置，必须按顺序执行 -> A -> B -> C -> D -> E -> F 执行。不能以测试资产、提交范围或旧报告跳过测试案例策略和检查点白盒审查。
...
| 配置项 | 用途 |
|---|---|
| `paths.docs.test_cases` | 测试案例目录 |
| `paths.docs.requirements` | 需求文档目录 |
| `patterns.test_case_files` | 测试案例文件检索模式 |
| `patterns.design_docs` | 设计文档检索模式 |
...
|---|---|---|
| 0 | `config-reader.md` | `project-config.yaml` |
