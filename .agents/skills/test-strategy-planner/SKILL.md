...
每个模块完成后立即增量写入报告，不等待后续模块完成：

- 模块 A 完成后，立即写入第 1 节“分析范围”和“分析前确认清单”。
- 模块 B 完成后，立即写入第 2 节“分层测试方案”和第 3 节“测试案例分层策略与需求-案例覆盖矩阵”，并锁定策略字段。
- 模块 C 完成后，立即写入第 4 节“自测检查点映射”和“待补充案例列表”。
- 模块 D 完成后，写入剩余章节（TODO、人工验证方案、风险提醒）并执行输出自检。
- 对话中简要说明已完成章节和当前结论；不要重复整份报告。后续步骤只能补充未完成章节，或因新增事实修订受影响的既有章节并说明修订原因。
...
先读取 `references/case-layering-strategy.md`，并按需读取 `references/small-model-reading.md`；只有用户确认纳入人工验证后，才读取 `references/manual-evidence.md`。
...
| B | `case-layering-strategy.md` | `manual-evidence.md`、`small-model-reading.md` |
