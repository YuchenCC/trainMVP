...
本 skill 以已确认的测试案例为主线，结合逻辑代码和共享自测检查点，产出分层测试策略、补充案例建议和执行准备。它不运行测试、不编写 JUnit5/JMeter/Playwright 脚本、不判定单测门禁最终通过，也不生成最终管理层测试覆盖总报告。
...
默认按 A -> B -> C -> E -> F 执行。不能以提交范围或旧报告跳过测试案例策略和检查点白盒审查；本 skill 不核对既有测试资产。

1. **模块 A：读取与范围确认**
2. **模块 B：测试案例分层策略**
3. **模块 C：自测检查点白盒审查与案例补充**
4. **模块 E：报告生成与输出自检**
5. **模块 F：最终对话、飞书发布与确认任务**
...
模块 C 的输出是检查点原文映射、代码判定、案例覆盖判断、补充案例建议和 TODO，作为工作状态交给模块 E。未完成不得进入模块 E。
...
- B/C 阶段完成后直接增量更新既定目标文件；E 不重新执行前序分析。
- 主报告汇总审核所需的策略、涉及检查点原文、补充案例建议、执行准备和风险；附件保留逐案例对照、测试实现建议、人工验证场景和检查点完整遍历附录。
...
最终回复保持简短：报告路径、范围、口径差异、分层策略与执行准备结论、检查点补充案例结论、输出自检结果、人工验证路径（如有）和必要的下一步建议。
...
跨模块工作状态与上下文续跑规则读取 `references/analysis-state.md`。B -> C -> E 的状态模型始终执行。只有上下文压力、大范围、多案例组、阶段性审查或跨会话续跑时才创建工作状态文件。上下文压力不能跳过模块 B/C 直接进入模块 E。
