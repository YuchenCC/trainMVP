...
| v1.2 | 2026-05-12 | 新增系统列表/系统用户/需求搜索三个查询接口；前端表单改为动态加载数据库真实数据，去掉所有 MOCK 数据；依赖选择改为关键词搜索+防抖；实时预览联动真实用户名称 |
...
**实现说明**：查询 System 表全量数据，按 `name` 升序排列，不关联其他表。前端用于表单中「归属系统」下拉框动态加载。

### 4.7 系统成员查询

```
GET /api/systems/:id/users
```
...
**实现说明**：通过 SystemMember 中间表查询指定系统下的所有成员。前端根据系统选择动态拉取成员列表，并按 `role` 过滤展示 BA/PM 下拉框。

| 场景 | HTTP状态码 | code | message |
|------|-----------|------|---------|
| 系统不存在 | 404 | NOT_FOUND | 系统不存在 |

### 4.8 需求搜索

```
GET /api/requirements/search?q={keyword}
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| q | string | 否 | 搜索关键词（模糊匹配需求编号和标题） |

**响应体**：

```typescript
[
  { id: string; reqCode: string; title: string; status: string }
]
```

**实现说明**：
- 空关键词或空白字符串直接返回空数组
- 使用 Prisma `contains` + `insensitive` 模式对 `reqCode` 和 `title` 做 OR 模糊匹配
- 按 `createdAt` 降序排列，最多返回 20 条
- 前端用于依赖选择——用户在搜索框中输入关键词 → 300ms 防抖 → 调用本接口 → 点击搜索结果添加到依赖列表
